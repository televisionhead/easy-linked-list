...
## Description
This linked list implementation aims to be easy to use, fast, type-generic, scalable, flexible, maintainable, and feature-rich. Not only does it strike a balance between these, but it excels in all aspects. It achieves polymorphism using static dispatch through the use of vtables indexed at compile time, allowing you to store and perform operations on any primitive or non-primitive data type, and even have multiple different data types in the same list (though you cannot call sort() on such a list). The purpose of this project is to provide other C programmers with a fast and simple linked list that they can easily add to their own projects and start using so they don't have to implement one themselves or import a heavy data structure library.

It is important to note that while my vtable implementation is by definition polymorphic in the sense that it chooses a function implementation based on type, it does so at compile time using static dispatch and thus does not incur the same performance costs that colloquial OOP polymorphism (as in C++, Java, etc.) implementations may entail, especially if such implementations involve complex inheritance hierarchies and/or do not require users of the API to explicitly pass type information to functions, as my implementation requires. When you start calling object member functions that override parent functions which override grandparent functions which override great grandparent functions, etc. all the way up a complex inheritance hierarchy, while deciding type information at runtime, that's when simple indexes into vtables turn into non-trivial vtable traversals, the compiler will start struggling to predict what function you will call next, and things will really start to slow down. Static dispatch requires the user of the API to pass one more parameter to describe the type, but this is a small price to pay for avoiding the slowdowns of dynamic dispatch polymorphism.

### Contributing
If you want to contribute a feature or optimization, fix a bug in the code or a problem with my testing methodology, or if you just have a suggestion, feel free to make a pull request or issue. Here is a general guideline for adding a new linked list operation:
