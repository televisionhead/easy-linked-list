...
typedef struct list {
	...
	void (*const add_struct)(struct list*, void*, int, void (*)(const void*), int (*comparator)(const void*, const void*)); //add a struct to the end of the list. just calls list_add_end with the values you pass for printer and comparator
	...
	struct list (*const split)(struct list*, int); //splits a list in half at the given index and returns a new list (the right half). dont forget to destroy the new list too when done
